...
FEATURES_PATH = ROOT / "data" / "current" / "week7_model_features.csv"
...
def main():
    ...
    if not (df["both_histories_established"].astype(int) == 1).all():
    ...
    df_missing = [c for c in set(wf + mf) if c not in df.columns]
    if df_missing:
        raise SystemExit(f"Missing features: {sorted(df_missing)}")

    p_a_win = winner.predict_proba(df[wf])[:, list(winner.classes_).index(1)]

    # Fighter-specific method model: orient features to candidate winner.
    xa = df[mf].copy()
    xb = -df[mf].copy()
    pa_method = aligned(method, xa, METHOD_ORDER)
    pb_method = aligned(method, xb, METHOD_ORDER)

    joint = np.zeros((len(df), 6), dtype=float)
    joint[:, 0:3] = p_a_win[:, None] * pa_method
    joint[:, 3:6] = (1.0 - p_a_win)[:, None] * pb_method
    joint /= joint.sum(axis=1, keepdims=True)

    # Direct six-class model retained as a disagreement diagnostic.
    df_direct = df[list(direct.feature_names_in_)]
    direct_joint = aligned(direct, df_direct, JOINT_ORDER)

    # Shadow duration + round layers.
    dur_probs = {}
    for market, model in duration.items():
        x = df[list(model.feature_names_in_)]
        dur_probs[market] = prob_one(model, x)

    round_x = df[list(round_model.feature_names_in_)]
    round_probs = aligned(round_model, round_x, ROUND_ORDER)

    rows = []
    sims = []
    rng_master = np.random.SeedSequence(20260922)
    child_seeds = rng_master.spawn(len(df))
    ...
    for i, r in df.iterrows():
        ...
        sim_joint = {JOINT_ORDER[j]: int((method_draws == j).sum()) for j in range(len(JOINT_ORDER))}
        sim_round = {ROUND_ORDER[j]: int((round_draws == j).sum()) for j in range(len(ROUND_ORDER))}
        sim_duration = {}
        for market in DURATION_MODELS:
            p = float(dur_probs[market][i])
            sim_duration[market] = int(rng.binomial(10000, p) / 1)
    ...
    for i, r in df.iterrows():
        detail.append({
            ...
            "winner": {
                str(r.fighter_a): float(p_a_win[i]),
                str(r.fighter_b): float(1-p_a_win[i]),
            },
    ...
    payload = {
        ...
        "note": "Duration and round layers remain shadow-only because their historical probability metrics did not beat the chronological naive baseline.",
        "fights": detail,
    }
